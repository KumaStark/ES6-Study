class Jq {
  ...
  // 作业要求
  animate(args, duration = 200, step = 200) {
    console.log("Animation Start!");
    let animateTimer = {};
    let deltaTime = Number(duration) / Number(step);
    // console.log(deltaTime);
    clearInterval(animateTimer.timer);
    let speedList = {};
    let currentList = {};
    for (let styleName in args) {
      speedList[styleName] = [];
      currentList[styleName] = [];
      for (let i = 0; i < this.length; i++) {
        let currentValue = window.getComputedStyle(this[i], null)[styleName];
        currentList[styleName][i] = this.cutUnit(currentValue);
        speedList[styleName][i] =
          (this.cutUnit(args[styleName]) - this.cutUnit(currentValue)) /
          Number(step);
      }
    }
    let stepAt = 1;
    animateTimer.timer = setInterval(() => {
      this.inAnimation = true;
      for (let i = 0; i < this.length; i++) {
        let newCSS = {};
        for (let styleName in args) {
          currentList[styleName][i] += speedList[styleName][i];
          newCSS[styleName] = currentList[styleName][i];
        }
        this.css(newCSS);
      }
      if (deltaTime * stepAt >= duration) {
